...
class CustomColumn extends ActionColumn {
	 ...
	 * @see buttons
	 */
	public $template = '{view} {update} {delete} {status}';
	/**
	 ...
	 */
	protected function initCustomButtons() {

		$this->initCustomButton('status', 'arrow-up', ['class' => 'btn btn-sm btn-icon  btn-round waves-effect waves-light waves-round']);
	}
	protected function initDefaultButtons() {
		$this->initDefaultButton('view', 'eye', ['class' => 'btn btn-sm btn-icon btn-success btn-round waves-effect waves-light waves-round']);
		$this->initDefaultButton('update', 'pencil', ['class' => 'btn btn-sm btn-icon btn-info btn-round waves-effect waves-light waves-round']);
		$this->initDefaultButton('delete', 'trash', [
			'data-confirm' => Yii::t('yii', 'Are you sure you want to delete this item?'),
			'data-method' => 'post',
			'class' => 'btn btn-sm btn-icon btn-danger btn-round waves-effect waves-light waves-round',
		]);
	}
	 ...
	 * @since 2.0.11
	 */
	protected function initCustomButton($name, $iconName, $additionalOptions = []) {
		if (!isset($this->buttons[$name]) && strpos($this->template, '{' . $name . '}') !== false) {
			$this->buttons[$name] = function ($url, $model, $key) use ($name, $iconName, $additionalOptions) {
				switch ($name) {

				case 'status':
					$str_status = (string) strtolower(strip_tags($model->status));
					$title = ($str_status === 'enabled' || $str_status == '1') ? Yii::t('app', 'Disable') : Yii::t('app', 'Enable');
					$iconName = ($str_status == 'enabled' || $str_status == '1') ? 'arrow-down' : 'arrow-up';
					$url .= "&val=";
					$url .= ($str_status == 'enabled' || $str_status == '1') ? '0' : '1';
					$c_class = $additionalOptions['class'];
					$c_class .= ($str_status == 'enabled' || $str_status == '1') ? ' btn-warning' : ' btn-success';
					$additionalOptions['class'] = $c_class;
					break;
				default:
					$title = ucfirst($name);
				}
