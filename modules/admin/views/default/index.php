...
if ($attendance) {
	...
	$attendanceStatus = $attendance['status'] == 'pause' ? 'resume' : 'pause';
            ...
            <!-- <span class="hour">00</span>:<span class="minute">00</span>:<span class="second">00</span> -->
                                ...
                                <div class="intro-y box p-2 mt-2 sm:mt-2 text-center" >
                                     ...
                                     <button class="btn bg-theme-9 text-white w-24 mb-2 mr-1" id="check-in" style="color: #fff!important" <?=$disabled == '' ? 'disabled' : ''?> onclick="attendance('check-in')"><?=Yii::t('app', 'Check In')?></button>
                                      ...
                                      <button class="btn bg-theme-1 text-white w-24 mb-2 mr-1" id="pause-time" style="color: #fff!important;text-transform: capitalize;"  <?=$disabled == 'disabled' ? 'disabled' : ''?> onclick="pauseResume()" data-act="<?=$attendanceStatus?>" ><?=Yii::t('app', $attendanceStatus)?></button>
...
if ($attendance) {
    ...
    var checkDate = "<?=date('c', (($attendance->start_date_time) - $attendance->work_duration))?>";
     var nowDate = "<?=(($attendance->stop_date_time) - ($attendance->start_date_time))?>";
    var isRunning = '<?=$attendance['status']?>';
    var workDuration = '<?=$attendance->work_duration?>';
    <?php
} else {
	?>
...
var inc = 0;
   currentTime = new Date(checkDate);
       // currentTime.setSeconds(currentTime.getSeconds() - parseInt(workDuration));
...
if(isRunning=='resume') {
   ...
   timeStart = setInterval(function() {
    ...
    function pauseResume() {
        ...
        var type = $('#pause-time').data('act');

        $.ajax({
            url: `<?=Yii::getAlias('@web')?>/site/attendance-status?type=${type}`,
            method: 'GET',
            dataType: 'json',
            success:function(d) {
                ...
                if(d.status=='resume') {
                    ...
                    $('#pause-time').attr('data-act','resume');
                    ...
                    currentTime = currentTime.getTime();
                     ...
                     $('#pause-time').attr('data-act','pause');
...
window.location.reload();
       ...
       $('#pause-time').attr('data-act','<?=$attendanceStatus?>');
                    $('#pause-time').text('<?=$attendanceStatus?>');
...
/*function pauseTime() {
    ...
    function timer() {
        ...
        var now = new Date();
         now.setSeconds(now.getSeconds() - nowDate);
       now = now.getTime();
