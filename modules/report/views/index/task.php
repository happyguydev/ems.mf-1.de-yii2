...
$this->title = Yii::t('app', 'Task Report');
...
?>
 <div class="grid grid-cols-12 gap-6 mt-5">
                    ...
                    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
                      ...
                      <?php if (Yii::$app->Utility->hasAccess('task_report', 'export')) {?>
...
?>
                        ...
                        <div class="hidden md:block mx-auto text-gray-600"></div>
                          <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
                            ...
                            <form id="myForm" method="get" action="<?=Yii::getAlias('@web')?>/report/index/task">
<div class="col-md-5">
...
<input type="text" id="end-date" class="input w-46 box pr-8 placeholder-theme-13 ml-3" name="enddate" value="<?=$enddate?>">
 ...
 <input type="submit" class="btn btn-success btn-round" value="<?=Yii::t('app', 'Submit')?>"/>
    <a href="<?=Yii::getAlias('@web')?>/report/index/task" class="btn btn-danger btn-round ml-2"><?=Yii::t('app', 'Reset')?></a>
 </div>
</div>
</form>
                        </div>
	...
	foreach ($model as $key => $value) {
		...
		?>
                                <tr>
                                    ...
                                    <td class="text-center"><a href="javascript:void(0)" class="text-theme-9" onclick="viewTask('<?=$value['id']?>')"><?=$value['name']?></a></td>
...
?>
  ...
  $('#pdf').on('click',function(){
    $("#export-table").tableHTMLExport({type:'pdf',filename:'task_report_<?=date('His')?>.pdf'});
    ...
    $(document).ready(function () {
        ...
        $("#export-table").table2excel({
            filename: "task_report_<?=date('His')?>.xls"
        });
    });
     });

   function viewTask(id) {
      window.open('<?=Yii::getAlias('@web')?>/task/view?id='+id,'_blank');
    }
