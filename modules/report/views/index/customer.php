...
?>
 <div class="grid grid-cols-12 gap-6 mt-5">
                    ...
                    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
                      <?php if (Yii::$app->Utility->hasAccess('customer_report', 'export')) {?>
...
?>
                        ...
                        <div class="hidden md:block mx-auto text-gray-600"></div>
                          <div class="w-full sm:w-auto mt-3 sm:mt-0 sm:ml-auto md:ml-0">
                            ...
                            <form id="myForm" method="get" action="<?=Yii::getAlias('@web')?>/report/index/customer">
<div class="col-md-5">
...
<input type="text" id="end-date" class="input w-46 box pr-8 placeholder-theme-13 ml-3" name="enddate" value="<?=$enddate?>">
 ...
 <input type="submit" class="btn btn-success btn-round" value="<?=Yii::t('app', 'Submit')?>"/>
    <a href="<?=Yii::getAlias('@web')?>/report/index/customer" class="btn btn-danger btn-round ml-2"><?=Yii::t('app', 'Reset')?></a>
 </div>
</div>
</form>
                        </div>
...
<table class="table table-report" id="export-table">
    <thead>
                                <tr>
                                    ...
                                    <th class="text-center whitespace-nowrap"><?=Yii::t('app', 'Customer No.')?></th>
                                    ...
                                    <th class="text-center whitespace-nowrap"><?=Yii::t('app', 'Address')?></th>
	...
	foreach ($model as $key => $value) {
		...
		?>
                                <tr>
                                    ...
                                    <td class="text-center"><?=$value['customer_number']?></td>
                                    <td class="text-center"><a href="javascript:void(0)" class="text-theme-9" onclick="viewCustomer('<?=$value['id']?>')"><?=$value['first_name'] . ' ' . $value['last_name']?></a></td>
...
?>
  ...
  $('#pdf').on('click',function(){
    $("#export-table").tableHTMLExport({type:'pdf',filename:'customer_report_<?=date('His')?>.pdf'});
    ...
    $(document).ready(function () {
        ...
        $("#export-table").table2excel({
            filename: "customer_report_<?=date('His')?>.xls"
        });
    });
     });

  function viewCustomer(id) {
      window.open('<?=Yii::getAlias('@web')?>/customer/view?id='+id,'_blank');
    }
